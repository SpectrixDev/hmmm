...
class EventHandler(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        message = GUILD_STATUS_MESSAGE.format(
            "Joined", guild, members, bots, humans)
        log.info(message)
        ...
        if guild.system_channel:
            ...
            embed.set_author(
                name="Confused? You should be, this bot makes no sense. Take this, it might help:")
            embed.add_field(
                name="Prefix", value="`%s`, or *mention me.*" % prefix)
            embed.add_field(name="Command help", value="%shelp" % prefix)
            embed.add_field(
                name="Support Server", value="[Join, it's fun here](https://discord.gg/Kghqehz)")
            embed.add_field(
                name="Upvote", value="[Click here](https://top.gg/bot/%d/vote)" % self.bot.user.id)
            embed.set_thumbnail(url=str(self.bot.user.avatar_url))
            embed.set_footer(
                text=f"Thanks to you, this monstrosity of a bot is now on {len(self.bot.guilds):,d} servers!", icon_url="https://media.giphy.com/media/ruw1bRYN0IXNS/giphy.gif")
    ...
    async def on_guild_remove(self, guild):
        ...
        message = GUILD_STATUS_MESSAGE.format(
            "Lefted", guild, member_count, bot_count, human_count)
        log.info(message)
        await self.webhook.send("```\n%s\n```" % message)
        await self.bot.update()
